...
<div class="max-w-3xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-8">
    <div class="mb-6 border-b border-gray-100 pb-4">
        <h2 class="text-xl font-bold text-gray-800">Edit Transaksi Keuangan</h2>
        <p class="text-sm text-gray-500">Perbarui data pencatatan kas UKM Anda.</p>
    </div>

    <form action="{{ route('admin-ukm.keuangan.update', $keuangan->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-5">
            <div class="grid grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" value="{{ $keuangan->tanggal }}" required class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Nominal (Rp) <span class="text-red-500">*</span></label>
                <input type="number" name="nominal" value="{{ $keuangan->nominal }}" required min="1" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Keterangan Transaksi <span class="text-red-500">*</span></label>
                <input type="text" name="keterangan" value="{{ $keuangan->keterangan }}" required class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
            </div>
